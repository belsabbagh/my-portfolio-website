import type { Project } from '../types';

const projects: Project[] = [
  {
    title: 'Hand Geometry Detection',
    description: '<p>A script that can be used to recognize a hand shape in an image.</p>',
    stack: ['Python', 'OpenCV'],
    href: 'https://github.com/belal-elsabbagh/hand-geometry-detection.git'
  },
  {
    title: 'Productivity Manager',
    description: `<p>A web app built to keep track of employee utilization on 
    department projects. This was requested by a software department 
    head who wanted a smooth way to track employees' utilization on projects 
    in his department.</p><p> My team and I built this in partial fulfillment for our 
    software engineering course. I implemented the models and controllers for 
    the API and implemented the UI for the login, dashboard, and charts pages.</p>`,
    stack: ['Laravel', 'MySQL', 'React.js', 'Chart.js'],
    href: 'https://github.com/belal-elsabbagh/swe-project.git'
  },
  {
    title: 'E-Commerce API',
    description: `<p>An API built to support an e-commerce website. 
    this was my first project with server-side JavaScript 
    and it helped me learn a lot about server-side 
    processes and better methods to structure an API.</p>`,
    stack: ['Node.js', 'Express', 'MongoDB', 'Mongoose', 'JWT'],
    href: 'https://github.com/belal-elsabbagh/e-commerce-app.git'
  }
];

export default projects;
